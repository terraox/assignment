...
async function seed() {
    try {
        ...
        const [employees] = await db_1.default.query('SELECT id FROM employees LIMIT 1');
        ...
        for (const task of dummyTasks) {
            await db_1.default.query(`INSERT INTO tasks (title, description, priority, status, start_date, due_date, assigned_employee_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [task.title, `Dummy description for ${task.title}`, task.priority, task.status, task.start, task.due, empId]);
        }
